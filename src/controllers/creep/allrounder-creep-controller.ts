...
import { priorityBuildFlag } from "../../constants/flag-constants";
...
export class AllRounderCreepController extends CreepController {
    ...
    protected getNewHighLevelTaskForCreep() {
        if (super.creepIsFull()) {
            // Try filling spawn energy or building depending on flags in room
            this.highLevelFillOrBuild();
        } else {
            this.pickupCollectOrMine();
        }
    }

    /** Either picks up dropped energy, collect energy from a container, or mines energy in that order of preference */
    private pickupCollectOrMine() {
        ...
        if (!pickupSucceeded) {
            ...
            if (!retrieveSucceeded) {
                ...
                this._creep.say("Forced to Mine");
    ...
    private highLevelFillOrBuild() {
        const flags = this._roomState.room.find(FIND_FLAGS);

        if (flags.find(flag => flag.name.includes(priorityBuildFlag))) {
            // Build first then fill
            if (this.modules.build.roomHasConstructionSites()) {
                // Try building
                super.setTask(CreepTasks.building);
                this.modules.build.buildConstructionSite();
